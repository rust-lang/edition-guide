...
There are two undocumented derive macros that have been removed from the prelude, `RustcEncodable` and `RustcDecodable`.
...
However, these derive macros are still available in the standard library, they just required to be imported from the older prelude now:
